...
const RED = "red",
  ...
  BLACK = "black",
  GRAY = "gray";
Object.defineProperty(window, "NULL", {
  value: null,
  writable: false,
  enumerable: true,
  configurable: true
...
for (let i in console) {
  Console.push(console[i].name || "");
  Console[console[i].name] = console[i];
  Console = Console.filter(Console => Console);
}
...
HTMLElement.prototype.newEvent = function() {
  return this.addEventListener(String(arguments[0] || arguments[0]), (arguments[1] || arguments[1]));
};

HTMLElement.prototype.removeEvent = function() {
  return this.addEventListener(String(arguments[0] || arguments[0]), function(event) {
    event.stopImmediatePropagation();
  }, true);
};

HTMLElement.prototype.removeAllEvents = function() {
  let cthis = this.cloneNode(true);
  this.parentNode.replaceChild(cthis, this);
};
 ...
 */
Window.prototype.NOT = function() {
  return !arguments[0];
};

Window.prototype.AND = function() {
...
Window.prototype.IsTrue = function() {
  if (arguments[0]) {
    return true;
  } else {
    return false;
...
Window.prototype.IsFalse = function() {
  if (arguments[0]) {
    return false;
  } else {
    return true;
...
Window.prototype.IsNaN = function() {
...
Window.prototype.IsString = function() {
...
Window.prototype.IsNumber = function() {
...
Window.prototype.IsBoolean = function() {
...
Window.prototype.IsUndefined = function() {
  if ((typeof arguments[0]) === "undefined" || arguments[0] === undefined) {
    return true;
  } else {
    return false;
  }
...
Window.prototype.IsNull = function() {
  if (arguments[0] === null) {
    return true;
  } else {
    return false;
...
Window.prototype.IsObject = function() {
...
Window.prototype.IsArray = function() {
...
setTimeout(function() {
  for (let i = 0; i < document.querySelectorAll("swift").length; i++) {
    ...
    r.src = /* "https://cdn.jsdelivr.net/gh/Parking-Master/Swifty.js@latest/" + String(document.querySelectorAll("swift")[i].getAttribute("version")) + "/engine.js"; */ "https://parkingmaster.w3spaces.com/swifty/engine.js";
...
function start_minimization(el) {
  document.onmousemove = function(e) {
    const y = e.clientY;
    document.querySelector(".swift-as-console").style.top = String(y) + "px";
    document.querySelector(".swift-as-console").style.height = "100%";
  };
...
<div class="swift-as-console" style="overflow-y: scroll; margin-left: -10px; z-index: 10000; position: fixed; bottom: 0 !important; width: 100%; height: 18%; font-family: sans-serif; color: rgb(204, 204, 204); background-color: #f9f9f9;">
  <div style="height: 5px; margin-top: -5px; cursor: n-resize; background-color: #e9e9e9;" title="Double-click to toggle log minimization" ondblclick="start_minimization(this)"></div>
    <div style="position: relative; font-family: Consolas, "Courier New", monospace; overflow: auto; height: 100%; padding-top: 5px;">
      <div title="${Date().split(" ").at(4)}" style="min-height: 16px;">
        <div class="swift-as-console-line" style="font-size: 12px; margin: 0px 8px; max-width: 100%; white-space: pre-wrap; overflow: auto; margin: 5px;"></div>
      </div>
    </div>
    <div style="position: absolute; top: 5px; right: 20px; color: rgb(17, 17, 17); border-radius: 4px; cursor: pointer; font-weight: normal; text-align: center; padding: 3px 5px; background-color: #e9e9e9; font-size: 12px;" title="Close Log" onclick="this.parentElement.style.display = 'none'; swiftConsoleOpen = true;">&times;</div>
</div>
`;
...
for (let i = 0; i < document.querySelectorAll("swift[type=pseudo-code]").length; i++) {
  document.querySelectorAll("swift[type=pseudo-code]").textContent = "";
}
